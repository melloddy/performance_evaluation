...
def init_arg_parser():
	parser = argparse.ArgumentParser(description="Test script for Calculate Performance Metrics (4 additional test performed in addition to the safety checks of the baseline code)")
	...
	parser.add_argument("--performance_report", help="Filename for json output from SC", type=str, required=True)
...
def per_run_performance(y_pred, pred_or_npy, tasks_table, y_true, tw_df, task_map, name, flabel):
	...
	json_perf = global_perf_from_json(args.performance_report)
	assert np.allclose([json_perf],[aucpr_mean], rtol=1e-05, atol=1e-05), \
		f"(Test #2 failed: Reported performance in {args.performance_report} ({json_perf}) not close to {flabel} ({aucpr_mean}) (tol:1e-05)"
...
def calculate_deltas(f1_results, f2_results, name, task_map):
	for idx, delta_comparison in enumerate(['locals','/deltas_global_performances.csv']):
		...
		if(delta_comparison == 'locals'):
			...
			assert np.allclose(tdf['aucpr'],[0]*len(tdf['aucpr']), rtol=1e-05, atol=1e-05), \
			f"(Test #3 failed! Calculated per-task deltas are not all close to zero (tol:1e-05)"
			...
			assert np.allclose(f2_results[idx]['aucpr_mean'], f1_results[idx]['aucpr_mean'], rtol=1e-05, atol=1e-05), "(Test #4 failed! Calculated global aggregation performance for f1 {f1_results[idx]['aucpr_mean'].values} and f2 {f2_results[idx]['aucpr_mean'].values} are not close (tol:1e-05)"
...
def yhat_allclose_check(yhat1,yhat2):
	nnz1 = yhat1.nonzero()
	nnz2 = yhat2.nonzero()
	try: allclose = np.allclose(yhat1[nnz1].astype('float32').todense(), yhat2[nnz2].astype('float32').todense(), rtol=1e-05, atol=1e-05)
	except: allclose = np.allclose(yhat1[nnz1].astype('float32'), yhat2[nnz2].astype('float32'), rtol=1e-05, atol=1e-05)
	vprint(f"Spearmanr rank correlation coefficient of f1 & f2 yhats = {spearmanr(yhat1[nnz1],yhat2[nnz2],axis=1)}")
	assert allclose, "(Test #1 failed! yhats not close between f1 & f2 (tol:1e-05)"
	return
...
def main(args):
	...
	yhat_allclose_check(f1_yhat,f2_yhat)
